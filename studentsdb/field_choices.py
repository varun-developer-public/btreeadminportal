DEGREE_CHOICES = [
    # UG Courses
    ('B.Tech / B.E.', 'B.Tech / B.E.'),
    ('B.Sc.', 'B.Sc.'),
    ('BCA', 'BCA (Bachelor of Computer Applications)'),
    ('BBA', 'BBA (Bachelor of Business Administration)'),
    ('BBM', 'BBM (Bachelor of Business Management)'),
    ('BMS', 'BMS (Bachelor of Management Studies)'),
    ('B.Com', 'B.Com (Commerce)'),
    ('MBBS', 'MBBS (Bachelor of Medicine and Bachelor of Surgery)'),
    ('BDS', 'BDS (Dental)'),
    ('BAMS', 'BAMS (Ayurveda)'),
    ('BHMS', 'BHMS (Homeopathy)'),
    ('B.Pharm', 'B.Pharm (Pharmacy)'),
    ('BPT', 'BPT (Physiotherapy)'),
    ('BA', 'BA (Arts & Humanities)'),
    ('BFA', 'BFA (Fine Arts)'),
    ('BPA', 'BPA (Performing Arts)'),
    ('B.Ed.', 'B.Ed. (Bachelor of Education)'),
    ('B.El.Ed.', 'B.El.Ed. (Elementary Education)'),
    ('BHM', 'BHM (Bachelor of Hotel Management)'),
    ('BA LLB / BBA LLB', 'BA LLB / BBA LLB'),
    ('LLB', 'LLB'),
    ('B.Des', 'B.Des (Bachelor of Design)'),
    ('BMM', 'BMM (Media and Mass Communication)'),
    ('BJMC', 'BJMC (Journalism and Mass Comm)'),

    # PG Courses
    ('M.Tech / M.E.', 'M.Tech / M.E.'),
    ('M.Sc.', 'M.Sc.'),
    ('MCA', 'MCA (Master of Computer Applications)'),
    ('MBA', 'MBA (Master of Business Administration)'),
    ('M.Com', 'M.Com (Commerce)'),
    ('PGDM', 'PGDM (Post Graduate Diploma in Management)'),
    ('MD / MS', 'MD / MS (Medical Specializations)'),
    ('MDS', 'MDS (Dental Specializations)'),
    ('M.Pharm', 'M.Pharm (Pharmacy)'),
    ('MPT', 'MPT (Physiotherapy)'),
    ('M.Phil', 'M.Phil'),
    ('Ph.D.', 'Ph.D. (Doctorate)'),
    ('MA', 'MA (Arts & Humanities)'),
    ('MFA', 'MFA (Fine Arts)'),
    ('MSW', 'MSW (Master of Social Work)'),
    ('M.Ed.', 'M.Ed. (Master of Education)'),
    ('LLM', 'LLM (Master of Law)'),
    ('M.Des', 'M.Des (Master of Design)'),
    ('MJMC', 'MJMC (Master in Journalism and Mass Communication)'),
]

BRANCH_CHOICES = [
    # Computer Science & IT
    ('Computer Science', 'Computer Science'),
    ('Computer Science & Engineering', 'Computer Science & Engineering'),
    ('Information Technology', 'Information Technology'),
    ('AI/ML', 'AI/ML'),
    ('Robotics', 'Robotics'),
    ('Data Science', 'Data Science'),

    # Management & Business
    ('Accounting / Finance', 'Accounting / Finance'),
    ('Marketing', 'Marketing'),
    ('HR', 'HR'),
    ('Finance', 'Finance'),
    ('Analytics', 'Analytics'),

    # Medical & Life Sciences
    ('Nursing', 'Nursing'),
    ('Pharmacy', 'Pharmacy'),
    ('Physiotherapy', 'Physiotherapy'),
    ('Biotechnology', 'Biotechnology'),
    ('Clinical Research / Public Health', 'Clinical Research / Public Health'),

    # Science & Research
    ('Physics', 'Physics'),
    ('Chemistry', 'Chemistry'),
    ('Mathematics', 'Mathematics'),
    ('Environmental Science', 'Environmental Science'),
    ('Microbiology / Zoology / Botany', 'Microbiology / Zoology / Botany'),

    # Arts & Humanities
    ('English', 'English'),
    ('History', 'History'),
    ('Economics', 'Economics'),
    ('Sociology', 'Sociology'),
    ('Psychology', 'Psychology'),
    ('Political Science', 'Political Science'),
    ('Fine Arts', 'Fine Arts'),
    ('Performing Arts', 'Performing Arts'),
    ('Social Work', 'Social Work'),

    # Education
    ('Education', 'Education'),
    ('Elementary Education', 'Elementary Education'),

    # Aviation & Hospitality
    ('Aviation', 'Aviation'),
    ('Hotel Management', 'Hotel Management'),
    ('Aviation Management', 'Aviation Management'),
    ('Travel & Tourism', 'Travel & Tourism'),

    # Law
    ('Law', 'Law'),

    # Engineering (Non-CS)
    ('Mechanical Engineering', 'Mechanical Engineering'),
    ('Civil Engineering', 'Civil Engineering'),
    ('EEE', 'Electrical and Electronics Engineering'),
    ('ECE', 'Electronics and Communication Engineering'),

    # Design & Creative
    ('Fashion Design', 'Fashion Design'),
    ('UI/UX Design', 'UI/UX Design'),
    ('Industrial Design', 'Industrial Design'),
    ('Media and Mass Communication', 'Media and Mass Communication'),
    ('Journalism and Mass Comm', 'Journalism and Mass Comm'),

    # Other
    ('STEM Programs', 'STEM Programs'),
]